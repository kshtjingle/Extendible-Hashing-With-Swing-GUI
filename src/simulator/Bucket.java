...
public class Bucket implements Serializable
{
	...
	public Bucket(int capacity, int newDepth, int bitPattern)
	{
		this.bucket = new int[capacity];
	...
	{
		this.bucket = new int[b.getCapacity()];
	...
	public void incDepth1()
	{
		...
		if (this.bitPattern == -1)
			...
			this.bitPattern = BitUtility.append1(this.bitPattern, this.depth);
	}
	...
	public boolean insert(Integer value)
	{
		...
		System.out.println("INSerting Capcity = " + getCapacity());
	 ...
	 */
	public int countProbes(Integer key)
	{
		int numProbes = 0;
	...
	public void shiftLeft(int start, int end, int numberOfShifts)
	{
		...
		for (int i = end - 1; i > end-numberOfShifts - 1; i--) {
			bucket[i] = -1;
		}
	}
	...
	/**
	 * Filters all values in the bucket that does not end with bitPattern into b.
	 * 
	...
	public void filter(Bucket b, int bitPattern)
	{
		...
		for(int i = 0; i < this.numWords; i++) {
			...
			int val = this.bucket[i];
			
			if(BitUtility.getRightMostBits(h.hash(val), this.depth) != bitPattern) {
				
				b.insert(val);
				
				erase(i, i);
				shiftLeft(i + 1, this.getCapacity(), 1);
				this.numWords--;
		...
		for (int i = 0; i < this.numWords; i++) {
			...
			if (BitUtility.getRightMostBits(h.hash(Integer.parseInt(word)), this.depth) != bitPattern) {
				b.insert(word);
	...
	public void printBucket()
	{
		...
		for(int i = 0; i < this.getCapacity(); i++) {
			...
			//if(this.bucket[i] != -1) {
				...
				bucketContents += String.valueOf(this.bucket[i]) + " ";
